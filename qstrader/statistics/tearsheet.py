...
class TearsheetStatistics(AbstractStatistics):
    ...
    def _plot_txt_curve(self, stats, ax=None, **kwargs):
        ...
        fs = 9
        ax.text(0.25, 8.5, 'Total Return', fontsize=fs)
        ax.text(7.50, 8.5, '{:.0%}'.format(tot_ret), fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 7.5, 'CAGR', fontsize=fs)
        ax.text(7.50, 7.5, '{:.2%}'.format(cagr), fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 6.5, 'Sharpe Ratio', fontsize=fs)
        ax.text(7.50, 6.5, '{:.2f}'.format(sharpe), fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 5.5, 'Sortino Ratio', fontsize=fs)
        ax.text(7.50, 5.5, '{:.2f}'.format(sortino), fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 4.5, 'Annual Volatility', fontsize=fs)
        ax.text(7.50, 4.5, '{:.2%}'.format(returns.std() * np.sqrt(252)), fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 3.5, 'R-Squared', fontsize=fs)
        ax.text(7.50, 3.5, '{:.2f}'.format(rsq), fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 2.5, 'Max Daily Drawdown', fontsize=fs)
        ax.text(7.50, 2.5, '{:.2%}'.format(dd_max), color='red', fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 1.5, 'Max Drawdown Duration', fontsize=fs)
        ax.text(7.50, 1.5, '{:.0f}'.format(dd_dur), fontweight='bold', horizontalalignment='right', fontsize=8)

        ax.text(0.25, 0.5, 'Trades per Year', fontsize=fs)
        ax.text(7.50, 0.5, '{:.1f}'.format(trd_yr), fontweight='bold', horizontalalignment='right', fontsize=8)
        ...
        if self.benchmark is not None:
            ...
            ax.text(9.75, 8.5, '{:.0%}'.format(tot_ret_b), fontweight='bold', horizontalalignment='right', fontsize=8)
            ax.text(9.75, 7.5, '{:.2%}'.format(cagr_b), fontweight='bold', horizontalalignment='right', fontsize=8)
            ax.text(9.75, 6.5, '{:.2f}'.format(sharpe_b), fontweight='bold', horizontalalignment='right', fontsize=8)
            ax.text(9.75, 5.5, '{:.2f}'.format(sortino_b), fontweight='bold', horizontalalignment='right', fontsize=8)
            ax.text(9.75, 4.5, '{:.2%}'.format(returns_b.std() * np.sqrt(252)), fontweight='bold', horizontalalignment='right', fontsize=8)
            ax.text(9.75, 3.5, '{:.2f}'.format(rsq_b), fontweight='bold', horizontalalignment='right', fontsize=8)
            ax.text(9.75, 2.5, '{:.2%}'.format(dd_max_b), color='red', fontweight='bold', horizontalalignment='right', fontsize=8)
            ax.text(9.75, 1.5, '{:.0f}'.format(dd_dur_b), fontweight='bold', horizontalalignment='right', fontsize=8)
    ...
    def _plot_txt_trade(self, stats, ax=None, **kwargs):
        ...
        fs = 9
        ax.text(0.5, 8.5, 'Trade Winning %', fontsize=fs)
        ax.text(9.5, 8.5, win_pct_str, fontsize=8, fontweight='bold', horizontalalignment='right')

        ax.text(0.5, 7.5, 'Average Trade %', fontsize=fs)
        ax.text(9.5, 7.5, avg_trd_pct, fontsize=8, fontweight='bold', horizontalalignment='right')

        ax.text(0.5, 6.5, 'Average Win %', fontsize=fs)
        ax.text(9.5, 6.5, avg_win_pct, fontsize=8, fontweight='bold', color='green', horizontalalignment='right')

        ax.text(0.5, 5.5, 'Average Loss %', fontsize=fs)
        ax.text(9.5, 5.5, avg_loss_pct, fontsize=8, fontweight='bold', color='red', horizontalalignment='right')

        ax.text(0.5, 4.5, 'Best Trade %', fontsize=fs)
        ax.text(9.5, 4.5, max_win_pct, fontsize=8, fontweight='bold', color='green', horizontalalignment='right')

        ax.text(0.5, 3.5, 'Worst Trade %', fontsize=fs)
        ax.text(9.5, 3.5, max_loss_pct, color='red', fontsize=8, fontweight='bold', horizontalalignment='right')

        ax.text(0.5, 2.5, 'Worst Trade Date', fontsize=fs)
        ax.text(9.5, 2.5, max_loss_dt, fontsize=8, fontweight='bold', horizontalalignment='right')

        ax.text(0.5, 1.5, 'Avg Days in Trade', fontsize=fs)
        ax.text(9.5, 1.5, avg_dit, fontsize=8, fontweight='bold', horizontalalignment='right')

        ax.text(0.5, 0.5, 'Trades', fontsize=fs)
        ax.text(9.5, 0.5, num_trades, fontsize=8, fontweight='bold', horizontalalignment='right')
    ...
    def _plot_txt_time(self, stats, ax=None, **kwargs):
        ...
        fs = 9
        ax.text(0.5, 8.5, 'Winning Months %', fontsize=fs)
        ax.text(9.5, 8.5, '{:.0%}'.format(mly_pct), fontsize=8, fontweight='bold',
                horizontalalignment='right')

        ax.text(0.5, 7.5, 'Average Winning Month %', fontsize=fs)
        ax.text(9.5, 7.5, '{:.2%}'.format(mly_avg_win_pct), fontsize=8, fontweight='bold',
                color='red' if mly_avg_win_pct < 0 else 'green',
                horizontalalignment='right')

        ax.text(0.5, 6.5, 'Average Losing Month %', fontsize=fs)
        ax.text(9.5, 6.5, '{:.2%}'.format(mly_avg_loss_pct), fontsize=8, fontweight='bold',
                color='red' if mly_avg_loss_pct < 0 else 'green',
                horizontalalignment='right')

        ax.text(0.5, 5.5, 'Best Month %', fontsize=fs)
        ax.text(9.5, 5.5, '{:.2%}'.format(mly_max_win_pct), fontsize=8, fontweight='bold',
                color='red' if mly_max_win_pct < 0 else 'green',
                horizontalalignment='right')

        ax.text(0.5, 4.5, 'Worst Month %', fontsize=fs)
        ax.text(9.5, 4.5, '{:.2%}'.format(mly_max_loss_pct), fontsize=8, fontweight='bold',
                color='red' if mly_max_loss_pct < 0 else 'green',
                horizontalalignment='right')

        ax.text(0.5, 3.5, 'Winning Years %', fontsize=fs)
        ax.text(9.5, 3.5, '{:.0%}'.format(yly_pct), fontsize=8, fontweight='bold',
                horizontalalignment='right')

        ax.text(0.5, 2.5, 'Best Year %', fontsize=fs)
        ax.text(9.5, 2.5, '{:.2%}'.format(yly_max_win_pct), fontsize=8,
                fontweight='bold', color='red' if yly_max_win_pct < 0 else 'green',
                horizontalalignment='right')

        ax.text(0.5, 1.5, 'Worst Year %', fontsize=fs)
        ax.text(9.5, 1.5, '{:.2%}'.format(yly_max_loss_pct), fontsize=8,
