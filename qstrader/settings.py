...
def from_file(fname=DEFAULT_CONFIG_FILENAME, testing=False):
    if testing:
        return TEST
    try:
        with open(os.path.abspath(os.path.expanduser(fname)), encoding='utf-8') as fd:
...
""" % s_conf)
        time.sleep(3)
        try:
            with open(os.path.abspath(os.path.expanduser(fname)), "w", encoding='utf-8') as fd:
                fd.write(s_conf)
        except IOError:
...
def from_var(csv_dir, out_dir):
    return munchify({
        "CSV_DATA_DIR": os.path.abspath(csv_dir),
